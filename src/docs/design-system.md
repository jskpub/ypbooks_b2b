...
> 출처: Figma「영풍문고 B2B 독서복지 플랫폼 — Design System」v2.3 (2026.09.25), Design System 섹션(node 228:754). Figma 변수·컴포넌트 실측값을 기준으로 작성했고, 남은 확인 사항은 **Known Gaps**에 정리했다.
...
- **Company** (`{colors.subsidy-company}` — #157a40) / 배경 `{colors.subsidy-company-soft}` #eaf7ef / 아이콘 `{colors.subsidy-company-icon}` #1c8a4b: 회사 지원금 잔액과 한도 영역.
...
**`side-button`** — 가로 스크롤 도서 목록 양옆의 50×50 원형 버튼. 흰 배경 + 1px `{colors.border-control}` + `Shadow/Floating`, caret 아이콘 20px. Disabled에는 그림자를 빼서 떠 있지 않음을 알린다. 목록 처음에서는 Left, 끝에서는 Right를 Disabled로. 아이콘만 있으므로 `aria-label`("이전 도서", "다음 도서")을 반드시 붙인다.
...
- 아이콘은 그 자리의 대상을 가리킨다(장바구니 `shopping-cart`, 검색 `magnifying-glass`, 목록 `books`).
...
- 금액은 천 단위 쉼표 + "원", 숫자는 tabular-nums로 정렬한다. 금액 하나는 서식을 쪼개지 않고 한 스타일로 쓴다.
- 목록이 비면 `empty-state`로 채운다. 빈 화면을 그대로 두지 않는다.
...
- **컴포넌트 서체와 스타일 연결:** 컴포넌트 텍스트가 Noto Sans / Noto Sans KR로 들어가 있고 스타일에 연결돼 있지 않았다. 화면 텍스트를 모두 v2 스타일에 연결했다(미연결 0개). 서체는 Pretendard GOV로 통일됐다.
- **Radius:** 문서 표(sm 6 / md 10 / lg 16)가 아니라 **Figma 변수·컴포넌트 값**(xs 2 / sm 4 / md 6 / lg 10 / full 999)을 채택하고 문서 표를 고쳤다. 16px는 쓰지 않는다.
- **Spacing 코드 이름:** 옛 이름(`--spacing-xs` 등)을 `--space-N`으로 통일했다.
- **Alert 제목 대비:** 옛 `state/danger-icon`(#e01e1a, 4.38:1로 AA 미달)에서 `{colors.danger}`(6.73:1)로 바꿨다. 회색 배경은 그대로 뒀다.
- **지원금 차감액 색:** 빨강을 기준으로 정했다. 경계는 "줄어드는 움직임은 빨강, 남은 양은 초록"이다(Colors → Subsidy 참고).
- **Badge Tone=Delivery:** 패턴에서 실제로 쓰이고 있어 유지하고, 시스템 변수에 연결한 뒤 세 번째 tone으로 문서화했다. 글자색을 #757575에서 `{colors.foreground-secondary}`로 바꿔 대비가 4.61:1에서 7.82:1이 됐다.
- **삭제 후보 색:** `state/success` 4종과 `state/warning`을 삭제했다. 마지막 사용처였던 Order Complete의 초록 알림을 Alert 기본 모습으로 통일했다. 성공·주의는 Alert 아이콘 모양으로만 구분한다. 이어서 빈 토큰이 된 `state/danger-icon`과 원시값 3개, 효과 스타일 2개도 지웠다(위 표 참고).
- **Input Error variant:** 컴포넌트에 State=Error(Empty, Filled) 2종을 추가했다.
- **Empty 상태:** `empty-state` 컴포넌트를 만들었다. 빈 장바구니는 MVP에서 반드시 생기므로 범위 밖으로 두지 않았다.
- **금액 서식:** 장바구니 금액 8개가 숫자와 "원"에 다른 굵기·색을 쓰고 있었다. 나머지 50개처럼 한 스타일로 합쳤다.
- **정가 표기:** 도서 카드 12px, 목록·장바구니 13px로 갈려 있던 것을 13px + 취소선으로 통일했다.
